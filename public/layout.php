...
function render_filters(array $filters): void
{
    foreach ($filters as $label => $options): ?>
        <label class="block">
            ...
            <select id="filter-<?= strtolower(str_replace(' ', '-', $label)) ?>" class="form-control filter-select w-full">
...
function render_estudiante_header(array $est): void
{
    ...
    <div class="flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div class="min-w-0 flex-1">
            ...
            <p class="text-xs text-slate-500"><?= e($est['codigo']) ?> · Periodo <?= e($est['ciclo']) ?> - Sección <?= e(clean_seccion($est['seccion'])) ?> · Periodo Académico 2026-I</p>
        </div>
...
<div class="flex min-h-screen bg-slate-50">
    ...
    <aside id="sidebar" class="hidden fixed inset-y-0 left-0 z-50 w-64 shrink-0 bg-[#1a3a6b] text-white transition-transform duration-300 lg:static lg:block">
        <div class="flex h-full w-full flex-col lg:sticky lg:top-0 lg:h-screen">
            <div class="w-full border-b border-white/10 px-4 py-4 lg:px-5 lg:py-5 flex items-center">
                <div class="flex items-center gap-2 flex-1 min-w-0 pr-8">
                    <div class="flex h-10 w-10 lg:h-12 lg:w-12 shrink-0 items-center justify-center">
                        <img src="<?= e(base_url('assets/images/logo_vrht.png')) ?>" alt="Logo VRHT" class="h-full w-full object-contain drop-shadow-md">
                    </div>
                    <div class="min-w-0 flex-1">
                        <p class="text-[10px] lg:text-[11px] font-bold leading-tight uppercase">IES "VÍCTOR RAÚL<br>HAYA DE LA TORRE"</p>
                        <p class="text-[9px] lg:text-[10px] mt-0.5 text-blue-200">Control de Asistencia</p>
                    </div>
                </div>
                <!-- Botón cerrar solo móvil -->
                <button type="button" class="lg:hidden absolute right-4 top-4 text-white/70 hover:text-white p-2" onclick="toggleSidebar()">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4">
                <?php foreach ($menu as $id => [$label, $icon]): ?>
                    <?php if ($id === 'dashboard'): 
                        $isDash = $module === 'dashboard';
                        $dashOpen = $isDash && isset($_GET['programa']) && $_GET['programa'] !== 'Todos';
                    ...
                    ?>
                        <div>
                            ...
                            <div id="submenu-dashboard" class="<?= $dashOpen ? '' : 'hidden' ?> mt-1 space-y-1 pl-10 pr-3 pb-2">
                                ...
                                foreach ($dashTabs as $t => $tLabel): ?>
                                    <a href="<?= e(base_url('index.php?m=dashboard&programa=' . $t)) ?>" class="block rounded-lg px-3 py-2 text-xs transition <?= $isDash && $dashTab === $t ? 'bg-white/20 font-semibold text-white' : 'text-blue-200 hover:bg-white/10' ?>">
                                        <?= e($tLabel) ?>
                                    </a>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    <?php elseif ($id === 'configuracion'): 
                        $cfgOpen = $module === 'configuracion';
                    ...
                    ?>
                        <div>
                            ...
                            <div id="submenu-config" class="<?= $cfgOpen ? '' : 'hidden' ?> mt-1 space-y-1 pl-10 pr-3 pb-2">
                                ...
                                foreach ($cfgTabs as $t => $tLabel): ?>
                                    <a href="<?= e(base_url('index.php?m=configuracion&tab=' . $t)) ?>" class="block rounded-lg px-3 py-2 text-xs transition <?= $cfgOpen && $activeTab === $t ? 'bg-white/20 font-semibold text-white' : 'text-blue-200 hover:bg-white/10' ?>">
                                        <?= e($tLabel) ?>
                                    </a>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    <?php elseif ($id === 'asistencia'): 
                        $asiOpen = $module === 'asistencia';
                        ...
                        <div>
                            ...
                            <div id="submenu-asistencia" class="<?= $asiOpen ? '' : 'hidden' ?> mt-1 space-y-1 pl-10 pr-3 pb-2">
                                ...
                                $asiTabs = [
                                    ...
                                    'docentes' => 'Asistencia Docentes',
                                ];
                                foreach ($asiTabs as $t => $tLabel): ?>
                                    <a href="<?= e(base_url('index.php?m=asistencia&tab=' . $t)) ?>" class="block rounded-lg px-3 py-2 text-xs transition <?= $asiOpen && $asiTab === $t ? 'bg-white/20 font-semibold text-white' : 'text-blue-200 hover:bg-white/10' ?>">
                                        <?= e($tLabel) ?>
                                    </a>
                                <?php endforeach; ?>
                            </div>
                ...
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <button type="button" data-notif-toggle class="relative rounded-lg border border-slate-200 bg-white p-2 text-slate-400 transition hover:text-[#1a3a6b]">
                            ...
                            <?php $unread_count = count(array_filter($notifications, fn($n) => !($n['is_read'] ?? false))); ?>
                            <?php if ($unread_count > 0): ?>
                                <span id="notif-badge" class="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white"><?= $unread_count ?></span>
                            <?php else: ?>
                        ...
                        <div id="notif-panel" class="absolute right-[-4rem] sm:right-0 top-full z-40 mt-2 hidden w-[320px] max-w-[calc(100vw-2rem)] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
                            ...
                            <div id="notif-list" class="max-h-72 divide-y divide-slate-100 overflow-y-auto">
                                <?php foreach ($notifications as $notif): ?>
                                    <div
                                        ...
                                        class="cursor-pointer px-4 py-3 transition hover:bg-slate-50 <?= !empty($notif['is_read']) ? 'opacity-60' : '' ?>"
                                    >
                                        <div class="flex items-start gap-2.5">
                                            <span class="mt-1.5 h-2 w-2 shrink-0 rounded-full <?= notif_dot_class($notif['tipo']) ?>"></span>
                                            <div class="min-w-0 flex-1">
                                                <p class="text-xs leading-snug <?= !empty($notif['is_read']) ? 'text-slate-500' : 'font-medium text-slate-800' ?>"><?= e($notif['msg']) ?></p>
        ...
        <form onsubmit="changePassword(event)" class="p-5">
            <div class="space-y-4">
                ...
                <label class="block">
                    ...
                    <input type="password" id="modal_new_password" name="new_password" required minlength="8" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[\W_]).{8,}" title="Debe contener al menos 8 caracteres, una mayúscula, una minúscula, un número y un carácter especial" class="form-control w-full" placeholder="Mín. 8 caracteres, 1 mayúscula, 1 número, 1 símbolo" oninput="checkPasswordStrength(this.value, 'modal')">
                </label>
                <div class="mt-2 text-[11px] sm:text-xs text-slate-500 grid grid-cols-1 sm:grid-cols-2 gap-1.5" id="pwd_rules_modal">
                    <div id="modal_rule_len" class="flex items-center gap-1"><i data-lucide="circle" class="w-3 h-3 shrink-0"></i> Mínimo 8 caracteres</div>
                    <div id="modal_rule_upper" class="flex items-center gap-1"><i data-lucide="circle" class="w-3 h-3 shrink-0"></i> Una mayúscula</div>
                    <div id="modal_rule_lower" class="flex items-center gap-1"><i data-lucide="circle" class="w-3 h-3 shrink-0"></i> Una minúscula</div>
                    <div id="modal_rule_num" class="flex items-center gap-1"><i data-lucide="circle" class="w-3 h-3 shrink-0"></i> Un número</div>
                    <div id="modal_rule_sym" class="flex items-center gap-1"><i data-lucide="circle" class="w-3 h-3 shrink-0"></i> Un símbolo (@, #, !, etc.)</div>
                </div>
            </div>
...
document.addEventListener('click', (event) => {
    ...
    if (notifMarkAll) {
        const ids = Array.from(document.querySelectorAll('[data-notif-item]')).map(item => item.dataset.notifItem);
        fetch('api/notificaciones.php', {
            method: 'POST',
            credentials: 'same-origin',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({mark_all: true, ids: ids})
    ...
    if (notifItem) {
        fetch('api/notificaciones.php', {
            ...
            credentials: 'same-origin',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({mark_id: notifItem.dataset.notifItem})
        }).catch(err => console.error(err));
