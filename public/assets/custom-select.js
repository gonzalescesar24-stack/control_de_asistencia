document.addEventListener('DOMContentLoaded', () => {
    const initCustomSelects = () => {
        document.querySelectorAll('select.form-control:not(.custom-select-initialized)').forEach(select => {
            ...
            dropdown.className = 'absolute z-50 mt-1 w-full whitespace-normal break-words rounded-lg border border-slate-200 bg-white py-1 shadow-lg opacity-0 pointer-events-none transition-opacity duration-200';
            ...
            const updateTriggerText = () => {
                ...
                if (selectedOption && selectedOption.text.trim() !== '') {
                    ...
                    text = select.options[0].text || '-- Seleccionar --';
                } else {
                    text = '-- Seleccionar --';
                }
            ...
            // Automatically detect when options or attributes change
