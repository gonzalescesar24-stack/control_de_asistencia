    ...
    if ($entity === 'programa') {
        match ($action) {
            'create' => (function() use ($data, $pdo) {
                ...
                log_audit($pdo, 'Configuración', 'CREAR', "Creado programa: {$nombre} ({$codigo})");
            ...
            'update' => (function() use ($data, $pdo) {
                ...
                log_audit($pdo, 'Configuración', 'MODIFICAR', "Actualizado programa ID {$id}: {$nombre}");
        ...
        match ($action) {
            'create' => (function() use ($data, $pdo) {
                ...
                log_audit($pdo, 'Configuración', 'CREAR', "Creado periodo académico: {$nombre}");
            ...
            'update' => (function() use ($data, $pdo) {
                ...
                log_audit($pdo, 'Configuración', 'MODIFICAR', "Estado periodo académico ID {$id} cambiado a: {$estado}");
            ...
            'delete' => (function() use ($data, $pdo) {
                ...
                $stmt->execute([$id]);
                log_audit($pdo, 'Configuración', 'ELIMINAR', "Eliminado periodo académico ID {$id}");
        ...
        match ($action) {
            'create' => (function() use ($data, $pdo) {
                $nombre = trim($data['nombre'] ?? '');
                if (!$nombre) throw new Exception('El nombre del periodo es requerido');
                $stmt = $pdo->prepare('INSERT INTO periodos_curriculares (modulo_id, nombre) VALUES (?, ?)');
        ...
        match ($action) {
            'create' => (function() use ($data, $pdo) {
                ...
                log_audit($pdo, 'Sesiones', 'CREAR', "Creada sesión: Periodo {$periodo}, Sección {$seccion}, Fecha {$fecha}");
        ...
        match ($action) {
            'update_regla_inasistencia' => (function() use ($data, $pdo) {
                ...
                $riesgo = (int) ($data['riesgo'] ?? 20);
                $stmt = $pdo->prepare('REPLACE INTO configuracion (clave, valor) VALUES (?, ?)');
                $stmt->execute(['regla_inasistencia', $porcentaje]);
                $stmt->execute(['regla_riesgo', $riesgo]);
                
                // Recalculate all student states based on the new rule
                $limiteInh = $porcentaje / 100;
                $limiteRie = $riesgo / 100;
                $stmtUpdate = $pdo->prepare("
                    UPDATE estudiantes 
                    SET estado = CASE 
                        WHEN (inasistencias / NULLIF(total_sesiones, 0)) >= ? THEN 'Inhabilitado'
                        WHEN (inasistencias / NULLIF(total_sesiones, 0)) >= ? THEN 'En riesgo'
                        ELSE 'Activo'
                    END
                ");
                $stmtUpdate->execute([$limiteInh, $limiteRie]);
                
                log_audit($pdo, 'Configuración', 'MODIFICAR', "Regla de inasistencias actualizada: Inhabilitado={$porcentaje}%, Riesgo={$riesgo}%");
            ...
            'update_tiempo_edicion' => (function() use ($data, $pdo) {
                ...
                log_audit($pdo, 'Configuración', 'MODIFICAR', "Tiempo de edición actualizado a {$horas} horas");
