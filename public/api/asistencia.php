    ...
    foreach ($asistencias as $asistencia) {
        ...
        if ($registroPrevio) {
            if ($registroPrevio['estado'] !== $estado) {
                ...
                $motivo_cambio = 'Modificación manual';

                if ($user['rol'] !== 'admin' && (time() - strtotime($registroPrevio['created_at']) > $segundosBloqueo)) {
                    if (empty($data['motivo_docente'])) {
                        throw new Exception("Modificación extemporánea: Se requiere justificación (Estudiante ID: $estudiante_id).");
                    }
                    $motivo_cambio = "Cambio extemporáneo: " . $data['motivo_docente'];
                }
