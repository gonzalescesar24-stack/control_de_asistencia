...
try {
    match ($action) {
        'create' => (function() use ($data, $pdo) {
            ...
            if ($correo) {
                $stmtCheckUsr = $pdo->prepare('SELECT COUNT(*) FROM usuarios WHERE correo = ?');
                $stmtCheckUsr->execute([$correo]);
                if ($stmtCheckUsr->fetchColumn() > 0) throw new Exception('El correo ya está en uso en el sistema');
            }
            ...
            log_audit($pdo, 'Docentes', 'CREAR', "Registrado docente: {$nombres} ({$codigo})");
        ...
        'update' => (function() use ($data, $pdo) {
            ...
            log_audit($pdo, 'Docentes', 'MODIFICAR', "Actualizado docente ID {$id}: {$nombres}");
        ...
        'toggle_status' => (function() use ($data, $pdo) {
            ...
            log_audit($pdo, 'Docentes', 'MODIFICAR', "Estado cambiado a {$newStatus} para docente ID {$id}");
