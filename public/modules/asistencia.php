...
<?php if ($tab === 'estudiantes'): ?>
    <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <!-- Filtros dinámicos -->
        <div class="mt-5 flex flex-col md:flex-row md:flex-wrap items-start md:items-end gap-3 w-full">
            <label class="block w-full md:w-auto">
                <span class="mb-1 block text-xs font-semibold text-slate-600">Periodo académico</span>
                <select id="sel-periodo" class="form-control w-full md:w-40">
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="sel-programa" class="form-control w-full md:w-64">
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="sel-unidad" class="form-control w-full md:w-64">
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="sel-seccion" class="form-control w-full md:w-32">
                    <option value="">-- Todas --</option>
            ...
            <label class="block relative w-full md:w-auto">
                <span class="mb-1 block text-xs font-semibold text-slate-600">Sesión académica</span>
                <select id="sel-sesion" class="form-control w-full md:w-48">
                    <option value=""></option>
                </select>
...
<?php else: ?>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="mt-5 flex flex-col md:flex-row md:flex-wrap items-start md:items-end gap-3 w-full">
            <label class="block w-full md:w-auto">
                <span class="mb-1 block text-xs font-semibold text-slate-600">Periodo académico</span>
                <select id="doc-sel-periodo" class="form-control w-full md:w-40">
                    <option value="">Todos los periodos</option>
                    <option value="2026-I" selected>2026-I</option>
                    <option value="2026-II">2026-II</option>
                    <option value="2025-II">2025-II</option>
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="doc-sel-docente" class="form-control w-full md:w-64">
                    <option value="">-- Todos --</option>
            ...
            <label class="block relative w-full md:w-auto">
                <span class="mb-1 block text-xs font-semibold text-slate-600">Sesión académica</span>
                <select id="doc-sel-sesion" class="form-control w-full md:w-64">
                    <option value=""></option>
                </select>
            </label>
        </div>
...
(function() {
    ...
    async function loadSesiones() {
        ...
        try {
            ...
            selSesion.innerHTML = '<option value="">-- Seleccionar --</option>';
            ...
            if (data.sesiones && data.sesiones.length > 0) {
                ...
                selSesion.innerHTML = '<option value="">No hay opciones disponibles</option>';
            }
        } catch (e) {
            selSesion.innerHTML = '<option value="">Error al cargar opciones</option>';
        } finally {
    ...
    async function loadEstudiantes() {
        ...
        try {
            ...
            // Los inputs siempre estarán habilitados para permitir la edición.
            ...
            if (recordCount) recordCount.textContent = `${data.estudiantes.length} registro${data.estudiantes.length !== 1 && data.estudiantes.length !== "1" ? "s" : ""}`;
    ...
    btnConfirmModal.addEventListener('click', () => {
        modalConfirm.classList.add('hidden');
        doSaveAsistencia(txtMotivo.value.trim());
    });
    // -------------------
    ...
    btnGuardar?.addEventListener('click', async () => {
        ...
        const selectedOpt = selSesion.options[selSesion.selectedIndex];
        const estadoSesion = selectedOpt?.dataset.estado;
        const userRole = '<?= e($user['rol'] ?? '') ?>';

        // Si la sesión está cerrada y es un docente, mostrar el modal de validación
        if (estadoSesion === 'Cerrada' && userRole !== 'admin') {
            ...
            // Administradores o sesiones no cerradas guardan directo
            doSaveAsistencia();
        }
    });

    async function doSaveAsistencia(motivoEspecial = null) {
        const sesionId = selSesion.value;
        ...
        try {
            ...
            const bodyData = { sesion_id: sesionId, asistencias };
            if (motivoEspecial) {
...
(function() {
    ...
    async function loadDocSesiones() {
        ...
        try {
            const res = await fetch('api/sesiones_api.php?' + params.toString());
            const data = await res.json();
            docSelSesion.innerHTML = '<option value="">-- Seleccionar --</option>';
