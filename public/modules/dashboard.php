...
$db_programas = fetch_all("SELECT codigo, nombre FROM programas WHERE estado = 'Activo'");
$programaMap = [];
foreach ($db_programas as $p) {
    $programaMap[$p['codigo']] = $p['nombre'];
}
...
$limiteInhabilitado = $cfgInasistencia / 100;
$limiteRiesgo = $cfgRiesgo / 100;
...
$totales = fetch_one("
    SELECT 
        COUNT(e.id) as total,
        SUM(CASE WHEN (e.inasistencias / NULLIF(e.total_sesiones, 0)) < $limiteRiesgo OR e.total_sesiones IS NULL OR e.total_sesiones = 0 THEN 1 ELSE 0 END) as activos,
        SUM(CASE WHEN (e.inasistencias / NULLIF(e.total_sesiones, 0)) >= $limiteRiesgo AND (e.inasistencias / NULLIF(e.total_sesiones, 0)) < $limiteInhabilitado THEN 1 ELSE 0 END) as riesgo,
        SUM(CASE WHEN (e.inasistencias / NULLIF(e.total_sesiones, 0)) >= $limiteInhabilitado THEN 1 ELSE 0 END) as inhabilitados
    FROM estudiantes e
...
$alertas = fetch_all("
    SELECT e.nombres, p.nombre as programa, ud.nombre as unidad, 
        CASE 
            WHEN (e.inasistencias / NULLIF(e.total_sesiones, 0)) >= $limiteInhabilitado THEN 'Inhabilitado'
            ELSE 'En riesgo'
        END as estado,
    ...
    WHERE (e.inasistencias / NULLIF(e.total_sesiones, 0)) >= $limiteRiesgo " . ($filtroNombre ? "AND p.nombre = ?" : "") . " 
...
$cards = [
    ...
    ['Estudiantes En Riesgo', $totales['riesgo'] ?? 0, '#ea580c', "Cercanos al {$cfgInasistencia}%"],
    ['Inhabilitados', $totales['inhabilitados'] ?? 0, '#dc2626', "Superaron {$cfgInasistencia}%"],
];
...
?>
<div class="space-y-6">
    <div class="flex flex-col sm:grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-6">
    ...
    <div class="flex flex-col gap-6 xl:grid xl:grid-cols-2">
        <div class="w-full rounded-xl border border-slate-200 bg-white p-6 shadow-sm xl:self-start">
