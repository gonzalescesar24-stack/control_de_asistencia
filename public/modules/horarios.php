...
<div class="space-y-4">
    <div class="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center w-full">
        <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto flex-1">
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <label class="relative block flex-1 md:w-64">
                    ...
                    <input id="filter-search" class="form-control filter-input w-full pl-9" placeholder="Buscar horario...">
                </label>
                <button type="button" data-modal-target="modal-nuevo-horario" class="md:hidden inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54] shrink-0">
        ...
        <div class="flex gap-2 w-full md:w-auto shrink-0 mt-2 md:mt-0 justify-end">
            <button type="button" data-modal-target="modal-importar-horarios" class="w-full md:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-[#1a3a6b] bg-white px-3 py-2 text-sm font-semibold text-[#1a3a6b] transition hover:bg-blue-50">
                <i data-lucide="upload" class="h-4 w-4"></i>
                <span>Importar CSV</span>
            </button>
            <button type="button" data-modal-target="modal-nuevo-horario" class="hidden md:inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54]">
                <i data-lucide="plus" class="h-4 w-4"></i>
    ...
    <div class="hidden md:block overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
            ...
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 shrink-0 whitespace-nowrap">
                <?= count($horarios) ?> registro<?= count($horarios) !== 1 ? 's' : '' ?>
            </span>
        </div>
    ...
    <?php foreach ($horarios as $index => $h): ?>
        <div class="bg-white rounded-xl border p-4 shadow-sm horario-row" data-search="<?= strtolower(e($h['dia_semana'] . ' ' . $h['hora_inicio'] . ' ' . $h['hora_fin'] . ' ' . $h['unidad'] . ' ' . $h['docente'])) ?>" data-programa="<?= e($h['programa']) ?>">
            ...
            <div class="text-xs text-slate-500 mb-2">Prog: <?= e($h['programa']) ?> | Periodo - Sec: <?= e($h['ciclo']) ?>-<?= e(clean_seccion($h['seccion'])) ?></div>
...
<!-- Modal Nuevo Horario -->
<div id="modal-nuevo-horario" data-modal class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-950/50 p-4 overflow-y-auto">
...
<div id="modal-importar-horarios" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
