            ...
            <table class="w-full text-sm">
                ...
                <?php foreach ($asistencias as $index => $a): ?>
                    <tr class="transition hover:bg-slate-50/50 asistencia-row" data-unidad="<?= e($a['unidad']) ?>" data-seccion="<?= e($a['seccion']) ?>" data-estado="<?= e($a['estado']) ?>">
                        ...
                        <td class="table-td text-center font-medium text-slate-600"><?= e($a['ciclo']) ?>-<?= e(clean_seccion($a['seccion'])) ?></td>
    ...
    <div class="md:hidden">
        <div class="mb-4 flex items-center justify-between">
            ...
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 mobile-table-count-badge shrink-0 whitespace-nowrap">
                <?= count($asistencias) ?> registro<?= count($asistencias) !== 1 ? 's' : '' ?>
            </span>
        </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col gap-3 asistencia-row" data-unidad="<?= e($a['unidad']) ?>" data-seccion="<?= e($a['seccion']) ?>" data-estado="<?= e($a['estado']) ?>">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-xs text-slate-500"><?= e($a['unidad']) ?> · <?= e($a['ciclo']) ?>-<?= e(clean_seccion($a['seccion'])) ?></p>
                    </div>
