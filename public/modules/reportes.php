...
<form action="<?= e(base_url('api/reporte_pdf.php')) ?>" method="GET" target="_blank">
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 items-end gap-4 w-full md:w-auto flex-1">
                <label class="block w-full">
                    <span class="mb-1.5 block text-xs font-semibold text-slate-700">Tipo de reporte</span>
                    <select name="tipo" class="form-control w-full bg-slate-50 focus:bg-white transition-colors">
                ...
                <label class="block w-full">
                    ...
                    <select name="programa_de_estudio" class="form-control w-full bg-slate-50 focus:bg-white transition-colors">
                ...
                <label class="block w-full">
                    ...
                    <select name="ciclo" class="form-control w-full bg-slate-50 focus:bg-white transition-colors">
                        <option value="Todos">-- Todos --</option>
                ...
                <label class="block w-full">
                    ...
                    <select name="unidad_didactica" class="form-control w-full bg-slate-50 focus:bg-white transition-colors">
                        <option value="Todos">-- Todos --</option>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto shrink-0 justify-end">
                <button type="submit" class="w-full sm:w-auto inline-flex justify-center items-center gap-2 rounded-lg bg-[#1a3a6b] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-[#142d54] focus:ring-2 focus:ring-[#1a3a6b]/50">
                    <i data-lucide="file-text" class="h-4.5 w-4.5"></i>
                    Generar PDF
                </button>
                <button type="submit" formaction="<?= e(base_url('api/export_csv.php')) ?>" class="w-full sm:w-auto inline-flex justify-center items-center gap-2 rounded-lg border border-emerald-200 bg-white px-4 py-2.5 text-sm font-semibold text-emerald-600 shadow-sm transition-all hover:border-emerald-300 hover:bg-emerald-50">
            ...
            ?>
                <div class="bg-white rounded-xl border p-4 shadow-sm transition hover:shadow-md">
                    ...
                    <div class="grid grid-cols-4 gap-1 bg-slate-50 rounded-lg p-2 text-center text-[10px] sm:text-xs">
                        <div class="min-w-0"><span class="text-slate-400 block truncate">Periodo - Sec</span><span class="font-semibold text-slate-700 truncate block"><?= e($e['ciclo']) ?>-<?= e(clean_seccion($e['seccion'])) ?></span></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        fetch('api/preview_reporte.php?' + urlParams)
            .then(res => res.json())
            .then(data => {
                countBadge.textContent = data.count + " registro" + (data.count !== 1 && data.count !== "1" ? "s" : "");
