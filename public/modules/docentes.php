...
?>
<div class="space-y-5">
    <div class="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center w-full">
        <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto flex-1">
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <label class="relative block flex-1 md:w-64">
                    <i data-lucide="search" class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"></i>
                    <input id="filter-search" class="form-control filter-input w-full pl-9" placeholder="Buscar docente...">
                </label>
                <button type="button" data-modal-target="modal-nuevo-docente" class="md:hidden inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54] shrink-0">
                    <i data-lucide="plus" class="h-4 w-4"></i>
                    <span class="sr-only sm:not-sr-only">Nuevo</span>
                </button>
            </div>
            <div class="flex flex-col md:flex-row md:flex-wrap items-start md:items-end gap-2 w-full md:w-auto">
                <select id="filter-programa" class="form-control filter-select w-full md:w-48">
                    <option value="">Todos los programas</option>
                    <?php foreach ($programas as $p): ?>
                        <option value="<?= e($p['nombre']) ?>"><?= e($p['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
                <select id="filter-estado" class="form-control filter-select w-full md:w-48">
        ...
        <div class="flex gap-2 w-full md:w-auto shrink-0 mt-2 md:mt-0 justify-end">
            <button type="button" data-modal-target="modal-importar-docentes" class="w-full md:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-[#1a3a6b] bg-white px-3 py-2 text-sm font-semibold text-[#1a3a6b] transition hover:bg-blue-50">
                <i data-lucide="upload" class="h-4 w-4"></i>
                <span>Importar CSV</span>
            </button>
            <button type="button" data-modal-target="modal-nuevo-docente" class="hidden md:inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54]">
                <i data-lucide="plus" class="h-4 w-4"></i>
    ...
    <div class="overflow-x-auto overflow-y-hidden min-h-[300px] rounded-xl border border-slate-200 bg-white shadow-sm hidden md:block">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
            ...
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 shrink-0 whitespace-nowrap">
                <?= count($docentes) ?> registro<?= count($docentes) !== 1 ? 's' : '' ?>
            </span>
        </div>
        <table class="w-full min-w-[1000px]">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="table-th whitespace-nowrap text-center">Periodo - Sección</th>
                    ...
                    <td class="table-td">
                        <div class="flex flex-col min-w-0">
                            ...
                            <?php if (!empty($d['unidad'])): ?>
                                <span class="truncate text-xs text-slate-500" title="<?= e($d['unidad']) ?>"><?= e($d['unidad']) ?> - <?= e($d['ciclo']) ?>-<?= e(clean_seccion($d['seccion'])) ?></span>
                            <?php else: ?>
        ...
        <?php foreach ($docentes as $index => $d): ?>
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm docente-row flex flex-col gap-3" data-search="<?= strtolower(e($d['codigo'] . ' ' . $d['nombres'] . ' ' . ($d['dni'] ?? '') . ' ' . ($d['correo'] ?? ''))) ?>" data-programa="<?= e($d['programa'] ?? '') ?>" data-estado="<?= e($d['estado'] ?? 'Activo') ?>">
                ...
                <div class="flex items-center gap-3 w-full">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-700 font-bold text-sm">
                        <?= e(user_initials($d['nombres'])) ?>
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-2 w-full">
                            <h3 class="font-bold text-slate-900 leading-tight mt-1"><?= e($d['nombres']) ?></h3>
                            <span class="rounded-full px-2 py-0.5 text-[10px] font-bold shrink-0 <?= badge_class($d['estado']) ?>"><?= e($d['estado']) ?></span>
                        </div>
                        <p class="mt-0.5 text-xs text-slate-500 truncate"><span class="font-mono text-[#1a3a6b] font-medium mr-1">#<?= $index + 1 ?> <?= e($d['codigo']) ?></span> &bull; DNI: <?= e($d['dni']) ?></p>
                    </div>
                </div>
                
                <!-- Body: Data Grid with Icons -->
                <div class="rounded-lg bg-slate-50 p-3 grid grid-cols-1 gap-2 border border-slate-100">
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        ...
                        <span class="truncate font-medium" title="<?= e($d['correo']) ?>"><?= e($d['correo']) ?: 'Sin correo' ?></span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        <i data-lucide="user" class="h-4 w-4 text-slate-400 shrink-0"></i>
                        <span class="truncate">Usuario: <span class="font-medium text-[#1a3a6b]"><?= e($d['usuario']) ?: 'N/A' ?></span></span>
                    </div>
                    <div class="flex items-start gap-2 text-sm text-slate-600">
                        <i data-lucide="graduation-cap" class="h-4 w-4 text-slate-400 shrink-0 mt-0.5"></i>
                        <div class="min-w-0 flex-1">
                            <span class="block font-medium truncate" title="<?= e($d['programa']) ?>"><?= e($d['programa']) ?: 'Sin programa' ?></span>
                            <?php if (!empty($d['unidad'])): ?>
                                <span class="text-xs text-slate-500 block truncate mt-0.5" title="<?= e($d['unidad']) ?>"><?= e($d['unidad']) ?> - <?= e($d['ciclo']) ?>-<?= e(clean_seccion($d['seccion'])) ?></span>
                            <?php else: ?>
                                <span class="text-xs text-slate-400 italic block mt-0.5">Sin Unidad Asignada</span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-1">
                    <button type="button" onclick="openViewModal(<?= htmlspecialchars(json_encode($d), ENT_QUOTES, 'UTF-8') ?>)" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold text-blue-600 shadow-sm transition hover:bg-blue-50">
                        <i data-lucide="eye" class="h-3.5 w-3.5"></i> Ver
                    </button>
                    <button type="button" onclick="openEditModal(<?= htmlspecialchars(json_encode($d), ENT_QUOTES, 'UTF-8') ?>)" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold text-orange-500 shadow-sm transition hover:bg-orange-50">
                        <i data-lucide="square-pen" class="h-3.5 w-3.5"></i> Editar
                    </button>
                    <button type="button" onclick="toggleStatus(<?= e($d['id']) ?>, '<?= e($d['estado']) === 'Activo' ? 'Inactivo' : 'Activo' ?>')" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold <?= e($d['estado']) === 'Activo' ? 'text-red-600 hover:bg-red-50' : 'text-emerald-600 hover:bg-emerald-50' ?> shadow-sm transition col-span-2 sm:col-span-1">
                        <i data-lucide="<?= e($d['estado']) === 'Activo' ? 'x-circle' : 'check-circle' ?>" class="h-3.5 w-3.5"></i> <?= e($d['estado']) === 'Activo' ? 'Desactivar' : 'Activar' ?>
                    </button>
                </div>
            </div>
        <?php endforeach; ?>
...
<!-- Modal Nuevo Docente -->
<div id="modal-nuevo-docente" data-modal class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto my-10 max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <form id="form-nuevo-docente" onsubmit="handleDocenteSubmit(event, 'create')">
            <div class="grid gap-4 p-6 md:grid-cols-2">
                ...
                <label><span class="mb-1 block text-sm font-medium text-slate-700">DNI</span><input type="text" name="dni" maxlength="8" pattern="\d{8}" title="Debe contener exactamente 8 dígitos" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 8);" class="form-control w-full"></label>
                ...
                <label><span class="mb-1 block text-sm font-medium text-slate-700">Periodo - Sección</span>
...
<div id="modal-editar-docente" data-modal class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto my-10 max-w-2xl rounded-2xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
        ...
        <form id="form-editar-docente" onsubmit="handleDocenteSubmit(event, 'update')">
            ...
            <div class="grid gap-4 p-6 md:grid-cols-2">
                ...
                <label><span class="mb-1 block text-sm font-medium text-slate-700">DNI</span><input type="text" name="dni" id="edit-dni" maxlength="8" pattern="\d{8}" title="Debe contener exactamente 8 dígitos" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 8);" class="form-control w-full"></label>
...
<!-- Modal Ver Docente -->
<div id="modal-ver-docente" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto mt-20 max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div class="p-6">
            ...
            <div class="mt-5 grid gap-3 md:grid-cols-2">
                ...
                <div class="rounded-lg bg-slate-50 p-3"><p class="text-xs text-slate-500">Periodo - Sección</p><p id="view-seccion" class="truncate font-semibold"></p></div>
...
<div id="modal-importar-docentes" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto mt-24 max-w-xl rounded-2xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
...
function openViewModal(docente) {
    ...
    document.getElementById('view-seccion').textContent = (docente.ciclo ? docente.ciclo + '-' : '') + (docente.seccion ? docente.seccion.slice(-1) : 'N/A');
