...
$enRiesgo = count(array_filter($estudiantes, fn($e) => pct($e) >= $cfgRiesgo && pct($e) < $cfgInasistencia));
$inhabilitados = count(array_filter($estudiantes, fn($e) => pct($e) >= $cfgInasistencia));
?>
<div class="space-y-4">
        ...
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <div class="mt-3 flex flex-wrap items-center gap-4 text-xs text-slate-600">
                    ...
                    <span class="flex items-center gap-2"><span class="h-2 w-2 rounded-full bg-red-500"></span>≥ <?= $cfgInasistencia ?>% → Inhabilitado</span>
    ...
    <div class="flex flex-col md:flex-row md:flex-wrap items-start md:items-end gap-3 w-full">
            <label class="block w-full md:w-auto">
                ...
                <select id="cal-filter-programa" class="form-control w-full md:w-64">
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="cal-filter-ciclo" class="form-control w-full md:w-40">
                    <option value="">-- Todos --</option>
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="cal-filter-unidad" class="form-control w-full md:w-64">
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="cal-filter-seccion" class="form-control w-full md:w-32">
                    <option value="">-- Todas --</option>
            ...
            <label class="block w-full md:w-auto">
                ...
                <select id="cal-filter-estado" class="form-control w-full md:w-48">
                    <option value="">-- Todos --</option>
    ...
    <div class="overflow-x-auto overflow-y-hidden min-h-[300px] rounded-xl border border-slate-200 bg-white shadow-sm hidden md:block">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4 bg-slate-50/50">
            <h2 class="text-lg font-bold text-[#0b2f63]">Calculo por estudiante — Regla del <?= $cfgInasistencia ?>%</h2>
            <div class="flex flex-wrap items-center gap-4 text-xs text-slate-600">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <span class="flex items-center gap-1"><span class="h-2 w-2 rounded-full bg-red-500"></span>Inhabilitado (≥<?= $cfgInasistencia ?>%)</span>
                </div>
            </div>
        </div>
        <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-100">
                <?php foreach ($estudiantes as $index => $estudiante): ?>
                    <?php
                    $pct = pct($estudiante);
                    $tone = $pct >= $cfgInasistencia ? 'red' : ($pct >= $cfgRiesgo ? 'orange' : 'emerald');
                    $bar = ['red' => 'bg-red-500 text-red-600', 'orange' => 'bg-orange-500 text-orange-600', 'emerald' => 'bg-emerald-500 text-emerald-600'][$tone];
                    $badge = $pct >= $cfgInasistencia ? 'Inhabilitado' : ($pct >= $cfgRiesgo ? 'En riesgo' : 'Activo');
                    ?>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm calculo-row flex flex-col gap-3"
                ...
                <div class="flex items-center gap-3 w-full">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-700 font-bold text-sm">
                        <?= e(user_initials($estudiante['nombres'])) ?>
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-2 w-full">
                            <h3 class="font-bold text-slate-900 leading-tight mt-1"><?= e($estudiante['nombres']) ?></h3>
                            <span class="rounded-full px-2 py-0.5 text-[10px] font-bold shrink-0 <?= badge_class($badge) ?>"><?= e($badge) ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-lg bg-slate-50 p-3 flex flex-col gap-3 border border-slate-100">
                    <div class="flex flex-col gap-2 text-sm text-slate-600">
                        <div class="flex items-start gap-2">
                            ...
                            <span class="font-medium truncate block" title="<?= e($estudiante['programa'] ?? 'N/A') ?>"><?= e($estudiante['programa'] ?? 'N/A') ?></span>
                        </div>
                        <div class="flex items-start gap-2">
                            <i data-lucide="book-open" class="h-4 w-4 text-slate-400 shrink-0 mt-0.5"></i>
                            <span class="font-medium truncate block" title="<?= e($estudiante['unidad'] ?? 'N/A') ?>"><?= e($estudiante['unidad'] ?? 'N/A') ?></span>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-slate-200 pt-2">
                        <div class="flex justify-between items-center mb-1 text-xs">
                            <span class="text-slate-500">Inasistencias: <span class="font-medium text-slate-700"><?= e($estudiante['inasistencias']) ?></span> de <?= e($estudiante['sesiones']) ?></span>
                            <span class="font-bold <?= e(substr($bar, strpos($bar, ' ') + 1)) ?>"><?= e($pct) ?>%</span>
                        </div>
                        <div class="h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
                            <div class="h-full rounded-full <?= e(strtok($bar, ' ')) ?>" style="width: <?= e(min($pct, 100)) ?>%"></div>
                        </div>
                    </div>
                </div>
            </div>
