...
<div class="space-y-6">
    ...
    <div class="hidden md:block overflow-x-auto min-h-[300px] rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <table class="w-full"><thead class="bg-slate-50"><tr><th class="table-th w-16 text-center">#</th><th class="table-th">Fecha</th><th class="table-th">Hora</th><th class="table-th">Programa</th><th class="table-th">Unidad</th><th class="table-th text-center">Periodo - Sección</th><th class="table-th">Periodo</th><th class="table-th">Estado</th><th class="table-th text-right">Acciones</th></tr></thead><tbody class="divide-y divide-slate-100">
            <?php if (empty($sesiones)): ?>
                <tr>
                    <td colspan="9" class="py-8 text-center text-slate-500">
                        ...
                        <p class="text-sm font-medium">No tienes sesiones asignadas para el periodo actual.</p>
                    </td>
                </tr>
            <?php else: ?>
                ...
                <tr class="transition hover:bg-slate-50/50">
                    ...
                    <td class="table-td text-center"><?= e($s['ciclo']) ?>-<?= e(clean_seccion($s['seccion'])) ?></td>
    ...
    <div class="md:hidden">
        <div class="mb-4 flex items-center justify-between">
            <h2 class="font-semibold text-[#1a3a6b]">Lista de Sesiones</h2>
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 mobile-table-count-badge shrink-0 whitespace-nowrap">
                <?= count($sesiones) ?> registro<?= count($sesiones) !== 1 ? 's' : '' ?>
            </span>
        </div>
        ...
        <?php if (empty($sesiones)): ?>
            <div class="bg-white rounded-xl border border-slate-200 p-8 text-center shadow-sm">
                <i data-lucide="calendar-x" class="mx-auto mb-3 h-10 w-10 text-slate-300"></i>
                <p class="text-sm font-medium text-slate-500">No tienes sesiones asignadas para el periodo actual.</p>
            </div>
        <?php else: ?>
        <?php foreach ($sesiones as $i => $s): $modalId = 'modal-sesion-' . $i; ?>
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col gap-3">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-xs text-slate-500 font-medium">Periodo - Sección: <?= e($s['ciclo']) ?>-<?= e(clean_seccion($s['seccion'])) ?></p>
                    </div>
...
<?php foreach ($sesiones as $i => $s): $modalId = 'modal-sesion-' . $i; ?>
    <div id="<?= e($modalId) ?>" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
        <div class="w-full mx-auto mt-20 max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div class="mt-5 flex flex-wrap gap-2">
                <a href="<?= e(base_url('index.php?m=registrar-asistencia&sesion_id=' . $s['id'])) ?>" class="rounded-lg bg-[#1a3a6b] px-4 py-2 text-sm font-semibold text-white hover:bg-[#142d54]">Registrar asistencia</a>
                <a href="<?= e(base_url('index.php?m=consultar-asistencia&sesion_id=' . $s['id'])) ?>" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Consultar asistencia</a>
            </div>
        </div>
    </div>
<?php endforeach; ?>
