...
<div class="space-y-4">
    <div class="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center w-full">
        <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto flex-1">
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <label class="relative block flex-1 md:w-64">
                    ...
                    <input id="filter-search" class="form-control filter-input w-full pl-9" placeholder="Buscar estudiante...">
                </label>
                <button type="button" data-modal-target="modal-nuevo-estudiante" class="md:hidden inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54] shrink-0">
            ...
            <div class="flex flex-col md:flex-row md:flex-wrap items-start md:items-end gap-2 w-full md:w-auto">
                ...
                <select id="filter-ciclo" class="form-control filter-select w-full md:w-32">
                ...
                <select id="filter-estado" class="form-control filter-select w-full md:w-40 ">
        ...
        <div class="flex gap-2 w-full md:w-auto shrink-0 mt-2 md:mt-0 justify-end">
            <button type="button" data-modal-target="modal-importar-estudiantes" class="w-full md:w-auto inline-flex items-center justify-center gap-2 rounded-lg border border-[#1a3a6b] bg-white px-3 py-2 text-sm font-semibold text-[#1a3a6b] transition hover:bg-blue-50">
                <i data-lucide="upload" class="h-4 w-4"></i>
                <span>Importar CSV</span>
            </button>
            <button type="button" data-modal-target="modal-nuevo-estudiante" class="hidden md:inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54]">
                <i data-lucide="plus" class="h-4 w-4"></i>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm estudiante-row flex flex-col gap-3" data-search="<?= strtolower(e($e['codigo'] . ' ' . $e['nombres'] . ' ' . ($e['dni'] ?? ''))) ?>" data-programa="<?= e($e['programa']) ?>" data-ciclo="<?= e($e['ciclo']) ?>" data-estado="<?= e($calculatedState) ?>">
                ...
                <div class="flex items-center gap-3 w-full">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-700 font-bold text-sm">
                        <?= e(user_initials($e['nombres'])) ?>
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-2 w-full">
                            <h3 class="font-bold text-slate-900 leading-tight mt-1"><?= e($e['nombres']) ?></h3>
                            <span class="rounded-full px-2 py-0.5 text-[10px] font-bold shrink-0 <?= badge_class($calculatedState) ?>"><?= e($calculatedState) ?></span>
                        </div>
                        <p class="mt-0.5 text-xs text-slate-500 truncate"><span class="font-mono text-[#1a3a6b] font-medium mr-1">#<?= $index + 1 ?> <?= e($e['codigo']) ?></span> &bull; DNI: <?= e($e['dni'] ?? '') ?></p>
                    </div>
                </div>
                ...
                <div class="rounded-lg bg-slate-50 p-3 flex flex-col gap-3 border border-slate-100">
                    <div class="grid grid-cols-2 gap-2 text-sm text-slate-600">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <span class="text-xs text-slate-500">Periodo <?= e($e['ciclo']) ?> - Sección <?= e(clean_seccion($e['seccion'])) ?></span>
                            </div>
                        </div>
                        <div class="flex items-start gap-2 border-l border-slate-200 pl-2">
                            <i data-lucide="book-open" class="h-4 w-4 text-slate-400 shrink-0 mt-0.5"></i>
                            <div class="min-w-0 flex-1">
                    ...
                    <div class="border-t border-slate-200 pt-2">
                        <div class="flex justify-between items-center mb-1 text-xs">
                            <span class="text-slate-500">Inasistencias: <span class="font-medium text-slate-700"><?= e($e['inasistencias']) ?></span> de <?= e($e['sesiones']) ?></span>
                ...
                <div class="grid grid-cols-2 gap-2 mt-1">
                    <button type="button" onclick="openViewModal(<?= htmlspecialchars(json_encode($e), ENT_QUOTES, 'UTF-8') ?>)" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold text-blue-600 shadow-sm transition hover:bg-blue-50">
                        <i data-lucide="eye" class="h-3.5 w-3.5"></i> Ver
                    </button>
                    <button type="button" onclick="openEditModal(<?= htmlspecialchars(json_encode($e), ENT_QUOTES, 'UTF-8') ?>)" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold text-orange-500 shadow-sm transition hover:bg-orange-50">
...
<div id="modal-nuevo-estudiante" data-modal class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto my-10 max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <form id="form-nuevo-estudiante" onsubmit="handleEstudianteSubmit(event, 'create')">
            <div class="grid gap-4 p-6 md:grid-cols-2">
                ...
                <label><span class="mb-1 block text-sm font-medium text-slate-700">Periodo</span>
...
<div id="modal-editar-estudiante" data-modal class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto my-10 max-w-2xl rounded-2xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
...
<!-- Modal Importar -->
<div id="modal-importar-estudiantes" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
...
<div id="modal-ver-estudiante" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto mt-20 max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div class="p-6">
            ...
            <div class="mt-5 grid gap-3 md:grid-cols-2">
                ...
                <div class="rounded-lg bg-slate-50 p-3"><p class="text-xs text-slate-500">Periodo - Sección</p><p id="view-cicloseccion" class="truncate font-semibold text-slate-800"></p></div>
...
function openViewModal(est) {
    ...
    document.getElementById('view-cicloseccion').textContent = (est.ciclo ? est.ciclo + '-' : '') + (est.seccion ? est.seccion.slice(-1) : 'N/A');
    ...
    document.getElementById('view-pct-text').textContent = `${pct}% límite superado`;
    
    const pctBar = document.getElementById('view-pct-bar');
    pctBar.style.width = Math.min((pct / <?= app_config('regla_inasistencia', 30) ?>) * 100, 100) + '%';
    
    let toneClass = 'bg-emerald-500';
    if (pct >= <?= app_config('regla_inasistencia', 30) ?>) toneClass = 'bg-red-500';
