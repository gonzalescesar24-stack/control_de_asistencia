...
$barTone = $porcentajeGlobal >= $cfgInasistencia ? 'bg-red-500' : ($porcentajeGlobal >= $cfgRiesgo ? 'bg-amber-500' : 'bg-emerald-500');
$pctTone = $porcentajeGlobal >= $cfgInasistencia ? 'text-red-600' : ($porcentajeGlobal >= $cfgRiesgo ? 'text-amber-600' : 'text-emerald-600');
?>
<div class="space-y-4">
    <?php
$cfgInasistencia = (int) app_config('regla_inasistencia', 30);
$cfgRiesgo = (int) app_config('regla_riesgo', 20);
 render_estudiante_header($est); ?>
        ...
        <div class="flex items-start gap-3">
            ...
            <div>
                ...
                <p class="mt-2 text-xs text-[#1a3a6b]/70">
                    Cuando el porcentaje acumulado alcanza o supera el <strong><?= $cfgInasistencia ?>%</strong>, el estudiante pasa a estado <strong>Inhabilitado</strong> en la unidad didáctica correspondiente.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="relative mb-6">
            <div class="h-5 overflow-hidden rounded-full bg-slate-100">
                <div class="h-full rounded-full <?= $barTone ?> transition-all duration-500" style="width: <?= min(($porcentajeGlobal / $cfgInasistencia) * 100, 100) ?>%"></div>
            </div>
            <div class="absolute top-0 flex h-5 items-center" style="left: <?= (20 / $cfgInasistencia) * 100 ?>%">
            ...
            <div class="relative mt-1">
                ...
                <span class="absolute -translate-x-1/2 text-[10px] text-amber-600" style="left: <?= (20 / $cfgInasistencia) * 100 ?>%"><?= $cfgRiesgo ?>% · Riesgo</span>
                <span class="absolute right-0 text-[10px] text-red-600"><?= $cfgInasistencia ?>% · Inhabilitado</span>
            </div>
        </div>
    ...
    <div class="hidden md:block overflow-x-auto min-h-[300px] rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
            ...
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 shrink-0 whitespace-nowrap">
                <?= count($unidades) ?> registro<?= count($unidades) !== 1 ? 's' : '' ?>
            </span>
        </div>
...
$cfgRiesgo = (int) app_config('regla_riesgo', 20);
 foreach ($unidades as $index => $u):
                        $p = $u['pct'];
                        $uBar = $p >= $cfgInasistencia ? 'bg-red-500' : ($p >= $cfgRiesgo ? 'bg-amber-500' : 'bg-emerald-500');
                        $uPct = $p >= $cfgInasistencia ? 'text-red-600' : ($p >= $cfgRiesgo ? 'text-amber-600' : 'text-emerald-600');
                    ?>
                            ...
                            <td class="table-td">
                                <div class="flex items-center gap-2">
                                    <div class="h-2 flex-1 overflow-hidden rounded-full bg-slate-100 min-w-16">
                                        <div class="h-full rounded-full <?= $uBar ?> transition-all duration-500" style="width: <?= min(($p / $cfgInasistencia) * 100, 100) ?>%"></div>
                                    </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col gap-3">
                ...
                <div class="flex items-center gap-3 w-full">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-2 w-full">
                            <span class="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-bold text-blue-700 truncate max-w-[65%] block">#<?= $index + 1 ?> <?= e($u['programa']) ?></span>
                            <span class="rounded-full px-2 py-0.5 text-[10px] font-bold shrink-0 <?= badge_class($u['estado']) ?>"><?= e($u['estado']) ?></span>
                        </div>
                        <h3 class="font-bold text-slate-900 leading-tight mt-1"><?= e($u['unidad']) ?></h3>
                    </div>
                </div>
                <!-- Body: Grid -->
                <div class="rounded-lg bg-slate-50 p-3 flex flex-col gap-3 border border-slate-100">
                    <div class="grid grid-cols-3 gap-2 text-center text-sm">
                        ...
                        <div class="border-l border-slate-200"><p class="text-[10px] uppercase tracking-wider text-slate-400">Asist.</p><p class="font-bold text-emerald-600"><?= $u['sesiones'] - $u['inasistencias'] ?></p></div>
                        <div class="border-l border-slate-200"><p class="text-[10px] uppercase tracking-wider text-slate-400">Faltas</p><p class="font-bold text-red-600"><?= $u['inasistencias'] ?></p></div>
                    </div>
                    <!-- Progress bar -->
                    <div class="mt-1 pt-3 border-t border-slate-200 flex items-center gap-3">
                        <span class="text-xs font-semibold text-slate-500">% Faltas:</span>
                        <div class="h-2 flex-1 overflow-hidden rounded-full bg-slate-200 min-w-16">
                            <div class="h-full rounded-full <?= $uBar ?> transition-all duration-500" style="width: <?= min(($p / $cfgInasistencia) * 100, 100) ?>%"></div>
                        </div>
                        <span class="w-8 text-right text-xs font-bold <?= $uPct ?>"><?= $p ?>%</span>
                    </div>
...
$cfgRiesgo = (int) app_config('regla_riesgo', 20);
    ...
    <div class="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
        ...
        <p class="text-xs text-amber-700">
            <strong>Regla del <?= $cfgInasistencia ?>%:</strong> Si el porcentaje de inasistencias en cualquier unidad didáctica es igual o superior al <?= $cfgInasistencia ?>%, el estudiante cambia automáticamente a estado <strong>Inhabilitado</strong> en esa unidad. Con ≥<?= $cfgRiesgo ?>% pasa a <strong>En riesgo</strong>.
        </p>
    </div>
</div>
