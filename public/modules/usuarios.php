...
<div class="space-y-5">
    <div class="flex flex-col md:flex-row gap-3 justify-between items-start md:items-center w-full">
        <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto flex-1">
            <div class="flex gap-2 w-full md:w-auto shrink-0">
                <label class="relative block flex-1 md:w-64">
                    ...
                    <input id="filter-search" class="form-control filter-input w-full pl-9" placeholder="Buscar usuario...">
                </label>
                <button type="button" data-modal-target="modal-nuevo-usuario" class="md:hidden inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-3 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54] shrink-0">
            ...
            <div class="flex flex-col md:flex-row md:flex-wrap items-start md:items-end gap-2 w-full md:w-auto">
                <select id="filter-rol" class="form-control filter-select w-full md:w-48">
        ...
        <div class="flex gap-2 w-full md:w-auto shrink-0 mt-2 md:mt-0 justify-end">
            <button type="button" data-modal-target="modal-nuevo-usuario" class="hidden md:inline-flex items-center justify-center gap-2 rounded-lg bg-[#1a3a6b] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#142d54]">
                <i data-lucide="plus" class="h-4 w-4"></i>
    ...
    <div class="overflow-x-auto overflow-y-hidden min-h-[300px] rounded-xl border border-slate-200 bg-white shadow-sm hidden md:block">
        <div class="flex items-center justify-between border-b border-slate-100 px-5 py-4 bg-slate-50/50">
            ...
            <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-bold text-blue-700 shrink-0 whitespace-nowrap">
                <?= count($usuarios) ?> registro<?= count($usuarios) !== 1 ? 's' : '' ?>
            </span>
        </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm user-row flex flex-col gap-3" data-rol="<?= e($u['rol']) ?>" data-estado="<?= e($u['estado']) ?>" data-search="<?= strtolower(e($u['nombre'] . ' ' . $u['usuario'] . ' ' . $u['correo'])) ?>">
                ...
                <div class="flex items-center gap-3 w-full">
                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-700 font-bold text-sm">
                        <?= e(user_initials($u['nombre'])) ?>
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center justify-between gap-2 w-full">
                            <h3 class="font-bold text-slate-900 leading-tight mt-1"><?= e($u['nombre']) ?></h3>
                            <span class="rounded-full px-2 py-0.5 text-[10px] font-bold shrink-0 <?= badge_class($u['estado']) ?>"><?= e($u['estado']) ?></span>
                        </div>
                        <p class="mt-0.5 text-xs text-slate-500 truncate"><span class="font-mono text-[#1a3a6b] font-medium mr-1">#<?= $index + 1 ?></span> &bull; <?= e($roleLabel[$u['rol']] ?? $u['rol']) ?></p>
                    </div>
                </div>
                ...
                <div class="rounded-lg bg-slate-50 p-3 grid grid-cols-1 gap-2 border border-slate-100">
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        ...
                        <span class="truncate font-medium"><?= e($u['usuario']) ?></span>
                    </div>
                    <div class="flex items-center gap-2 text-sm text-slate-600">
                        <i data-lucide="mail" class="h-4 w-4 text-slate-400 shrink-0"></i>
                        <span class="truncate" title="<?= e($u['correo']) ?>"><?= e($u['correo']) ?></span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-2 mt-1">
                    <button type="button" onclick="openEditModal(<?= htmlspecialchars(json_encode($u), ENT_QUOTES, 'UTF-8') ?>)" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold text-blue-600 shadow-sm transition hover:bg-blue-50">
                        <i data-lucide="square-pen" class="h-3.5 w-3.5"></i> Editar
                    </button>
                    <button type="button" onclick="resetPassword(<?= e($u['id']) ?>)" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold text-slate-600 shadow-sm transition hover:bg-slate-50">
                        <i data-lucide="refresh-cw" class="h-3.5 w-3.5"></i> Clave
                    </button>
                    <button type="button" onclick="toggleStatus(<?= e($u['id']) ?>, '<?= e($u['estado']) === 'Activo' ? 'Inactivo' : 'Activo' ?>')" class="flex items-center justify-center gap-1.5 rounded-md border border-slate-200 bg-white py-1.5 text-xs font-semibold <?= e($u['estado']) === 'Activo' ? 'text-red-600 hover:bg-red-50' : 'text-emerald-600 hover:bg-emerald-50' ?> shadow-sm transition">
                        <i data-lucide="<?= e($u['estado']) === 'Activo' ? 'x-circle' : 'check-circle' ?>" class="h-3.5 w-3.5"></i> <?= e($u['estado']) === 'Activo' ? 'Desactivar' : 'Activar' ?>
                    </button>
                </div>
                <?php endif; ?>
            </div>
...
<!-- Modal Nuevo Usuario -->
<div id="modal-nuevo-usuario" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto mt-24 max-w-xl rounded-2xl bg-white shadow-2xl">
        ...
        <form id="form-nuevo-usuario" onsubmit="handleUserSubmit(event, 'create')">
            <div class="grid gap-4 p-6 md:grid-cols-2">
                ...
                <label><span class="mb-1 block text-sm font-medium text-slate-700">Rol</span><select name="rol" onchange="toggleFields(this, 'form-nuevo-usuario')" class="form-control w-full"><option value="estudiante">Estudiante</option><option value="docente">Docente</option></select></label>
                ...
                <div class="campos-academicos md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 hidden">
                    ...
                    <label class="campo-dni"><span class="mb-1 block text-sm font-medium text-slate-700">DNI *</span><input type="text" name="dni" maxlength="8" pattern="\d{8}" title="Debe contener exactamente 8 dígitos" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 8);" class="form-control w-full"></label>
                    ...
                    <label class="campo-ciclo"><span class="mb-1 block text-sm font-medium text-slate-700">Periodo *</span>
...
<div id="modal-editar-usuario" data-modal class="fixed inset-0 z-50 hidden bg-slate-950/50 p-4 overflow-y-auto">
    <div class="w-full mx-auto mt-24 max-w-xl rounded-2xl bg-white shadow-2xl">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
        ...
        <form id="form-editar-usuario" onsubmit="handleUserSubmit(event, 'update')">
            ...
            <div class="grid gap-4 p-6 md:grid-cols-2">
                ...
                <label><span class="mb-1 block text-sm font-medium text-slate-700">Rol</span><select name="rol" id="edit-rol" onchange="toggleFields(this, 'form-editar-usuario')" class="form-control w-full"><option value="estudiante">Estudiante</option><option value="docente">Docente</option></select></label>
